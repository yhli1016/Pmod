...
m.add_mod('ICC/2018.1.163', preset='void',
...
m.add_mod('IMPI/2018.1.163', preset='void',
...
for version in openmpi_versions:
    m.add_mod(version, preset='mod', destination='/opt/%s' % version,
              depend=['ICC/2018.1.163'],
              conflict=list(set(openmpi_versions).difference({version})))
    m.add_mod(version, preset='void', conflict=['IMPI/2018.1.163'])
...
m.add_mod('PYXAID', preset='path', destination='/opt/PYXAID/bin',
          depend=['anaconda2', 'ICC/2018.1.163', 'IMPI/2018.1.163'],
...
m.add_mod('gpaw/1.4.0', preset='path', destination='/opt/gpaw/1.4.0/bin',
          ...
          depend=['anaconda2', 'ASE/3.16.2', 'ICC/2018.1.163',
                  'IMPI/2018.1.163', 'openblas/0.2.20'],
          conflict=openmpi_versions)
...
m.add_mod('qe/6.3', preset='path', destination='/opt/qe/6.3/bin',
          depend=['ICC/2018.1.163', 'IMPI/2018.1.163'],
          conflict=openmpi_versions)
